...
class Statambulatoria extends Model
{
    ...
    public function __construct($medicalOrganization)
    {
        $this->medicalOrganization = $medicalOrganization;
        /* old shit выборка по типу введения анкеты
        $sql = 'SELECT type, COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND type<>"" GROUP BY type';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $medicalOrganization ));
        foreach ($res as $value) {
            $this->countbytype[$value['type']] = (int)$value['count'];
        }
        */
        
        $sql = 'SELECT id FROM medicalOrganizations WHERE name=:medicalOrganization';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $medicalOrganization ));
        $this->id = $res[0]['id'];
    ...
    {
        $sql = 'SELECT COUNT(*) as count_true FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND ' . $row . '="Да"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $count_true = (int)$res[0]['count_true'];
        $sql = 'SELECT COUNT(*) as count_false FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND ' . $row . '="Нет"';
    ...
    {
        $row = 'row44';
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND ' . $row . ' LIKE "менее 7 календарных дней%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $a = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_21_6 + $this->mzambulatoria->mzambulatoria_25_6;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND ' . $row . ' LIKE "7 календарных дней%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $b = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_21_5 + $this->mzambulatoria->mzambulatoria_25_5;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND ' . $row . ' LIKE "10 календарных дней%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $c = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_21_4 + $this->mzambulatoria->mzambulatoria_25_4;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND ' . $row . ' LIKE "12 календарных дней%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $d = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_21_3 + $this->mzambulatoria->mzambulatoria_25_3;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND ' . $row . ' LIKE "13 календарных дней%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $e = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_21_2 + $this->mzambulatoria->mzambulatoria_25_2;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND ' . $row . ' LIKE "14 календарных дней%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ02(){
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row35 LIKE "%к врачу общей практики%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'];
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row35 LIKE "%к врачу-терапевту участковому%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = $arr[1] + (int)$res[0]['count'];
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row35 LIKE "%к врачу-педиатру участковому%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        ...
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row35 LIKE "%к врачу-специалисту%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[2] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_02_4;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row35 LIKE "%другое%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_3_1_value()
    {
        ...
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND ' . $row . ' LIKE "%не назначалось%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ56() {
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row65 LIKE "%за счет ОМС, бюджет%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_56_1;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row65 LIKE "%за счет ДМС%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[2] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_56_2;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row65 LIKE "%на платной основе%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ58() {
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row67 LIKE "%раз в месяц%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_58_1;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row67 LIKE "%раз в квартал%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[2] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_58_2;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row67 LIKE "%раз в полугодие%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[3] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_58_3;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row67 LIKE "%раз в год%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[4] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_58_4;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row67 LIKE "%не обращаюсь"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ59() {
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row68 LIKE "%раз в месяц%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_59_1;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row68 LIKE "%раз в квартал%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[2] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_59_2;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row68 LIKE "%раз в полугодие%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[3] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_59_3;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row68 LIKE "%раз в год%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[4] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_59_4;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row68 LIKE "%не обращаюсь"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ61() {
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row70 LIKE "%положительный%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_61_1;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row70 LIKE "%отрицательный%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ27() {
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row46 LIKE "%по телефону%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_27_1;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row46 LIKE "%с использованием сети Интернет%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[2] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_27_2;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row46 LIKE "%в регистратуре лично%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[3] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_27_3;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row46 LIKE "%лечащим врачом на приеме при посещении%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ28() {
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row47 LIKE "%не дозвонился%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_28_1;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row47 LIKE "%не было талонов%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[2] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_28_2;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row47 LIKE "%не было технической возможности записаться в электронном виде%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[3] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_28_3;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row47 LIKE "%другое%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ37() {
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row56 LIKE "I группа%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_37_1;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row56 LIKE "II группа%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[2] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_37_2;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row56 LIKE "III группа%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[3] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_37_3;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row56 LIKE "%ребенок-инвалид%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ38()
    {
        ...
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row58 LIKE "%отсутствие выделенного места стоянки автотранспортных средств для инвалидов%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_39_1;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row58 LIKE "%отсутствие пандусов, поручней%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[2] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_39_2;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row58 LIKE "%отсутствие подъемных платформ (аппарелей)%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[3] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_39_3;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row58 LIKE "%отсутствие адаптированных лифтов%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[4] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_39_4;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row58 LIKE "%отсутствие сменных кресел-колясок%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[5] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_39_5;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row58 LIKE "%отсутствие информационных бегущих строк%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[6] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_39_6;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row58 LIKE "%отсутствие информации шрифтом Брайля%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[7] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_39_7;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row58 LIKE "%отсутствие доступных санитарно-%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[8] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_39_8;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row58 LIKE "%отсутствие сопровождающих работников%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ19()
    {
        ...
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row43 LIKE "%информацию о состоянии здоровья%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_24_1;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row43 LIKE "%рекомендации по диагностике%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[2] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_24_2;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row43 LIKE "%дали выписку%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[3] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_24_3;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row43 LIKE "%рецепт%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[4] = (int)$res[0]['count'] + $this->mzambulatoria->mzambulatoria_24_4;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row43 LIKE "%другое%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ06() {
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row39 LIKE "%прием%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        ...
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row39 LIKE "%вызов%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ07() {
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row40 LIKE "%24 часа%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        ...
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row40 LIKE "%12 часов%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        ...
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row40 LIKE "%8 часов%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        ...
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row40 LIKE "%6 часов%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        ...
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row40 LIKE "%3 часа%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        ...
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row40 LIKE "%менее 1 часа%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ35(){
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row54 LIKE "%состояние гардероба%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[2] = (int)$res[0]['count'] + $this->mzabmulatoria->mzambulatoria_35_2;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row54 LIKE "%отсутствие свободных мест ожидания%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'] + $this->mzabmulatoria->mzambulatoria_35_1;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row54 LIKE "%состояние туалета%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[3] = (int)$res[0]['count'] + $this->mzabmulatoria->mzambulatoria_35_3;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row54 LIKE "%отсутствие питьевой воды%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[4] = (int)$res[0]['count'] + $this->mzabmulatoria->mzambulatoria_35_4;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row54 LIKE "%санитарные условия%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[5] = (int)$res[0]['count'] + $this->mzabmulatoria->mzambulatoria_35_5;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row54 LIKE "%детских колясок%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ63(){
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row72 LIKE "%я сам (а)%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'] + $this->mzabmulatoria->mzabmulatoria_63_1;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row72 LIKE "%персонал медицинской организации%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
    ...
    public function get_MZ64(){
        $arr = array();
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row73 LIKE "%письменная благодарность%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[1] = (int)$res[0]['count'] + $this->mzabmulatoria->mzabmulatoria_64_1;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row73 LIKE "%цветы%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[2] = (int)$res[0]['count'] + $this->mzabmulatoria->mzabmulatoria_64_2;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row73 LIKE "%подарки%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[3] = (int)$res[0]['count'] + $this->mzabmulatoria->mzabmulatoria_64_3;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row73 LIKE "%услуги%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
        $arr[4] = (int)$res[0]['count'] + $this->mzabmulatoria->mzabmulatoria_64_4;
        $sql = 'SELECT COUNT(*) as count FROM ' . static::TABLE . ' WHERE medicalOrganization=:medicalOrganization AND row73 LIKE "%деньги%"';
        $db = \App\Db::instance();
        $res = $db->queryRaw($sql, array('medicalOrganization' => $this->medicalOrganization ));
